...
var TOKEN = '31d0d698e1e38ef47a38330ffd1e1bd333EDC881A6B629370AF8C95B3B8F8CCB038EAF67'
...
function initUIData() {
  ...
  units.forEach(function(unit) {
    ...
    //session.unitsX.push({name:unit.getName(),id:unit.getId()});
...
function initMap(loc = [4.6, -74], zoo = 17) {
  ...
  L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
    ...
    attribution: '&copy; <a href="https://www.linkedin.com/in/fesus/">Fesus Rocuts</a> Full Stack Engineer'
  }).addTo(map);
...
function defaultCheck(uid = 0, name=""){
  setTimeout(function(){
      $('#units').val(uid);
      getGeofencesByUnitId(uid);
      setTimeout(function(){
        actIconItem(uid);
        getGeofencesByUnitId(uid);
      },10000);
...
function actIconItem(uid = 0){
  let items = $(".leaflet-marker-pane > img")
  if (items != undefined){
    items = items.filter(item => items[item].src.search("" + uid) > -1);
    if (items.length == 1){
